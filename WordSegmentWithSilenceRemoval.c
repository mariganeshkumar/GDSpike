 ...
 *  Last modified:  Thu 24-Apr-2014 15:05:33 by hema
...
main(int argc, char *argv[]) {
  ...
  FILE             *energyFile,*spectraFile,*peaksFile,
                   *sigFile,*ctrlFile,*digitFile, *cepFile;
  ...
  mfft = ceil(log((float)samples*2)/log(2.0)); 
  nfft = (int) pow(2,mfft); 
  nfby2 = nfft/2;
  ...
  peakArr = (int *) malloc((nfft+1)*sizeof(int)); 
  valleyArr = (int *) malloc((nfft+1)*sizeof(int)); 
...
printf("came here\n");
  ...
  for (i = 1; i <=2; i++) {
    dervSmth[i] = dervSmth[3];
    dervSmth[nfby2-i+1] = dervSmth[i];
    negDervSmth[i] = negDervSmth[3];
    negDervSmth[nfby2-i+1] = negDervSmth[i];
  }
  for (i = 3; i < nfby2-3; i++) {
    med[1] = negDerv[i-2];
    med[2] = negDerv[i-1];
